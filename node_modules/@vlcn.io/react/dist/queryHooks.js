...
//   ]);
// }
export function useQuery(ctx, query, bindings, postProcess, updateTypes = allUpdateTypes, _rowid_) {
    const stateMachine = useRef(null);
    const lastCtx = useRef(ctx);
    ...
    if (stateMachine.current == null || lastCtx.current !== ctx) {
        ...
        stateMachine.current = new AsyncResultStateMachine(ctx, query, bindings, postProcess, updateTypes, _rowid_);
    }
    useEffect(() => {
        return () => {
            stateMachine.current?.dispose();
    ...
    const [lastBindings, setLastBindings] = useState();
    ...
    return useSyncExternalStore(stateMachine.current.subscribeReactInternals, stateMachine.current.getSnapshot);
}
export function useQuery2(ctx, query, bindings, postProcess, updateTypes = allUpdateTypes, _rowid_) {
...
let queryTxHolder = null;
let queryId = 0;
let txAcquisition = null;
class AsyncResultStateMachine {
    ctx;
    query;
    bindings;
    postProcess;
    ...
    respondToBindingsChange = (bindings) => {
        if (this.disposed) {
            return;
        }
        let i = 0;
    ...
    respondToDatabaseChange = (updates) => {
        if (this.disposed) {
    ...
    prepare() {
        ...
        this.stmt = null;
        const preparePromise = this.prepareAndGetUsedTables().then(([stmt, queriedTables]) => {
            ...
            if (this._rowid_ != null) {
                ...
                this.dbSubscriptionDisposer = this.ctx.rx.onPoint(this.queriedTables[0], this._rowid_, this.respondToDatabaseChange);
            }
            else {
                this.dbSubscriptionDisposer = this.ctx.rx.onRange(queriedTables, this.respondToDatabaseChange);
            }
            return stmt;
        });
    ...
    fetch(rebind) {
        ...
        const fetchInternal = () => {
            ...
            const doFetch = (releaser, tx) => {
                return stmt
                    ...
                    if (pendingQuery === myQueryId) {
                        pendingQuery = null;
                        txAcquisition = null;
                        tx.exec("RELEASE use_query_" + queryTxHolder).then(releaser, releaser);
                    }
            ...
            fetchPromise = txAcquisition.then(([releaser, tx]) => doFetch(releaser, tx));
...
function usedTables(db, query) {
    return db.tablesUsedStmt.all(null, query).then((rows) => {
        return rows.map((r) => r[0]);
    });
}
function dataShallowlyEqual(left, right) {
